...
import com.bithealth.model.RequetParam;
...
public class SessionFilter extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request,HttpServletResponse response,FilterChain chain) throws IOException,ServletException
	{
	    ...
	    if(head != null && (null != requestUri) && !"/".equals(requestUri) && !submitMehtod.equals("POST")
	    		...
	    		&& !requestUri.startsWith("information/selectHealthEducationDetail")
	    		&& !requestUri.startsWith("attachedurl")
	    		&& !requestUri.startsWith("member/getPhoneVerificationCode")
	    		&& !requestUri.startsWith("member/register")
	    		&& !requestUri.startsWith("member/forgotThenResetPassword")
	    		&& !requestUri.startsWith("file/findAppDownloadPage")
	    		&& !requestUri.startsWith("member/findAppserverUrlByPhoneNumber")
	    		&& !requestUri.startsWith("member/authenticationPhoneNumber")
	    		&& !requestUri.startsWith("member/mergeInfomation")
	    		&& !requestUri.startsWith("doctor/login") 
	    		&& !requestUri.startsWith("doctor/logout")
	    		&& !requestUri.startsWith("measure/mergerBloodSugar")
	    		&& !requestUri.startsWith("measure/mergerBloodPressure")
	    		&& !requestUri.startsWith("measure/mergerMini")
	    		&& !requestUri.startsWith("measure/mergerThreeInOne")
	    		){
	    ...
	    if(null == requestUri || "/".equals(requestUri) || "index.jsp".equals(requestUri)|| requestUri.startsWith("js")|| submitMehtod.equals("GET")
	    	...
	    	if(requestUri.startsWith("measure/mergerBloodSugar") 
	    			||requestUri.startsWith("measure/mergerBloodPressure") 
	    			||requestUri.startsWith("measure/mergerMini") 
	    			||requestUri.startsWith("measure/mergerThreeInOne") 
	    	 ){ //4 个接口需要验证token
	    		try{
		    		RequetParam requetParam = JSONObject.parseObject(jsonStr, RequetParam.class);
		    		String token = Util.calculatToken(Util.transBean2TreeMap(requetParam));
		    		if(token == null ||  !token.equals(requetParam.getToken())){
						value.setStatusCode(5);
						value.setMessage("token错误");
						PrintWriter out = response.getWriter();
						out.println(JSON.toJSONString(value));						
						return;
		    		}
