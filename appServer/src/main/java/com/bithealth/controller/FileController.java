...
public class FileController extends BaseSpringController{
	...
	/**
	 * @Description: 查看app下载链接页面
	 * @author:      liuxiaoqin
	 * @version      V1.0  
	 * @Createdate:  2016年12月22日 
	 */
	@RequestMapping(value = "/findAppDownloadPage", method = RequestMethod.GET)
    public ModelAndView selectAppDownloadPage(HttpServletRequest request,HttpServletResponse response) throws IOException{
		ModelAndView mav = new ModelAndView();
		JSONResult<Object> value = new JSONResult<Object>();
		try{
			Map<String,Object> map = new HashMap<String, Object>();
			map.put("iosAddress", MessageUtil.getValue("iosDownloadAddress"));
			map.put("androidAddress", MessageUtil.getValue("androidDownloadAddress"));
			mav.addObject("data", map);
			mav.setViewName("appDownload"); 
			return mav;
    	}catch(Exception e){
    		PrintWriter out = response.getWriter();
    		value.setStatusCode(2);
    		value.setMessage(MessageUtil.getValue("error.select.data"));
			logger.info("查看app下载链接页面"+e);
			value.setSuccess(false);
			out.write(JSON.toJSONString(value));
			return null;
    	}
