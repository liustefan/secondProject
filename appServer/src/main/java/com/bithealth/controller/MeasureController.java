...
import com.bithealth.measureCore.enmu.BloodPresResultTips;
import com.bithealth.measureCore.enmu.BloodSugarResultTips;
...
public class MeasureController extends BaseSpringController{
    ...
    public void insertBloodPressure(HttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	try{
    		...
    		osbpExample.createCriteria().andMemberidEqualTo(memberId).andTesttimeEqualTo(TimeUtil.datetimeFormat2.parse(testTime)).andDeltagEqualTo("0");
    		osbpExample.setOrderByClause("Docentry DESC");
    		List<Osbp> newOsbpList = bloodPressureService.selectByExample(osbpExample);
    		if(newOsbpList != null && newOsbpList.size() >0){
    			Osbp newOsbp = newOsbpList.get(0);
    			value.setData(convertOsbp(newOsbp));
    		}
    		/*重新获取osbp  end*/
    		value.setStatusCode(0);
    ...
    public void insertBloodSugar(HttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	try{
    		...
    		obsrExample.createCriteria().andMemberidEqualTo(memberId).andTesttimeEqualTo(TimeUtil.datetimeFormat2.parse(testTime)).andDeltagEqualTo("0");
    		obsrExample.setOrderByClause("Docentry DESC");
    		List<Obsr> newObsrList = bloodSugarService.selectByExample(obsrExample);
    		if(newObsrList != null && newObsrList.size() >0){
    			Obsr newObsr = newObsrList.get(0);
    			value.setData(convertObsr(newObsr));
    		}
    		/*重新获取obsr  end*/
    		value.setStatusCode(0);
    ...
    public void insertMini(HttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	try{
    		...
    		oecgExample.createCriteria().andMemberidEqualTo(memberId).andDeltagEqualTo("0").andMeastimeEqualTo(TimeUtil.datetimeFormat2.parse(measureTime)).andFsEqualTo((short)150);
    		oecgExample.setOrderByClause("Docentry DESC");
    		List<Oecg> newOecgList = electrocardioService.selectByExample(oecgExample);
    		if(newOecgList != null && newOecgList.size()>0){
    			Oecg newOecg = newOecgList.get(0);
    			value.setData(convertOecg(newOecg));
    		}
    		/*重新获取oecg  end*/
			value.setStatusCode(0);
    ...
    public void insertThreeInOne(MultipartHttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	try{
    		...
    		oppgExample.createCriteria().andMemberidEqualTo(memberId).andDeltagEqualTo("0").andMeasuretimeEqualTo(TimeUtil.datetimeFormat2.parse(measureTime));
    		oppgExample.setOrderByClause("Docentry DESC");
    		List<Oppg> newOppgList = pulseService.selectByExample(oppgExample);
    		if(newOppgList != null && newOppgList.size()>0){
    			Oppg newOppg = newOppgList.get(0);
    			map.put("oppg", convertOppg(newOppg));
    		}
    		
    		OecgExample oecgExample = new OecgExample();
    		oecgExample.createCriteria().andMemberidEqualTo(memberId).andDeltagEqualTo("0").andMeastimeEqualTo(TimeUtil.datetimeFormat2.parse(measureTime)).andFsEqualTo((short)250);
    		oecgExample.setOrderByClause("Docentry DESC");
    		List<Oecg> newOecgList = electrocardioService.selectByExample(oecgExample);
    		if(newOecgList != null && newOecgList.size()>0){
    			Oecg newOecg = newOecgList.get(0);
    			map.put("oecg", convertOecg(newOecg));
    		}
    		/*重新获取oecg和oppg  end*/
    		value.setData(map);
			value.setStatusCode(0);
...
//			}
	...
	public Map<String,Object> convertOsbp(Osbp osbp)throws Exception{
		...
		map.put("measAbnTips", BloodPresResultTips.getTipsName(Integer.valueOf(osbp.getAbnormal())));
		return map;
	}
	
	/**
	 * @Description: 转化血糖obsr属性名 
	 * @author:      liuxiaoqin
	 * @version      V1.0  
	 * @Createdate:  2016年9月21日 
	 */
	public Map<String,Object> convertObsr(Obsr obsr)throws Exception{
		...
		map.put("measAbnTips", BloodSugarResultTips.getTipsName(Integer.valueOf(obsr.getAnalysisresult())));
		return map;
	}
