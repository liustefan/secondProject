...
//	/**
//	 * @Description: 根据健教任务id查询健教任务详情
//	 * @author:      liuxiaoqin
//	 * @param        
//	 * @version      V3.0  
//	 * @Createdate:  2017年1月05日 
//	 */
//	@RequestMapping(value = "/selectHealthEducationDetail", method = RequestMethod.POST)
//    public void selectHealthEducationDetial(HttpServletRequest request,HttpServletResponse response) throws IOException{
//		PrintWriter out = response.getWriter();
//		JSONResult<Object> value = new JSONResult<Object>();
//		try{
//			String appUser = (String)request.getAttribute("appUserStr");
...
//	    	String otherParam = (String)request.getAttribute("otherParamStr");
//			JSONObject StrObject = JSON.parseObject(otherParam);
//			Integer heducationid = StrObject.getInteger("heducationid");
//			if(heducationid == null || heducationid <= 0){
//	    		value.setStatusCode(1);
//				value.setMessage("健教ID【"+heducationid+"】应为正整数");
//				logger.info("健教ID【"+heducationid+"】应为正整数");
//				value.setSuccess(false);
//				out.write(JSON.toJSONString(value));
//				return; 
//	    	}
//    		
//    		/*调用获取健教详情    begin*/
//			HealthEducation info = healthFacedeService.selectHealthById(heducationid);
//			if(info != null ){
//				Map<String, Object> map = new HashMap<String, Object>();
//				Byte sourceway = info.getSourceway();
//				map.put("content", info.getContent());
//				map.put("sourceway", sourceway);
//				if(sourceway != null && sourceway == 2){
//					Integer healthId = info.getSourceid();
//					if(healthId != null && healthId >0){
//						HealthnewsInfo healthInfo = cmsFacedeService.selectNewsIngfoById(healthId);
//						if(healthInfo != null && healthInfo.getStatustype() != 4){
//							Map<String,Object> mapInfo = convertHealthnewsInfo(healthInfo,user.getUserId());
//							map.put("healthInfo", mapInfo);
//						}
//					}
//				}
//				value.setSuccess(true);
//				value.setStatusCode(0);
//				value.setMessage("获取健教内容详情成功");
//				value.setData(map);
//				logger.info("获取健教内容详情成功");
//			}else{
//				value.setSuccess(false);
//				value.setStatusCode(3);
//				value.setMessage("没有健教内容详情");
//				logger.info("没有健教内容详情");
//			}
//			/*调用获取健教详情    end*/
//		}catch(Exception e){
//			value.setSuccess(false);
...
//	}
	...
	 * @Createdate:  2017年1月05日 
	 */
	@RequestMapping(value = "/selectHealthEducationDetail", method = RequestMethod.GET)
    public ModelAndView selectHealthEducationDetail(HttpServletRequest request,HttpServletResponse response) throws IOException{
		ModelAndView mav = new ModelAndView();
		JSONResult<Object> value = new JSONResult<Object>();
    	try{
    		String heducationidStr = request.getParameter("heducationid");
    		if(StringUtils.isEmpty(heducationidStr) == true || Integer.valueOf(heducationidStr) <= 0){
    			PrintWriter out = response.getWriter();
	    		value.setStatusCode(1);
				value.setMessage("参数heducationid【"+heducationidStr+"】应为正整数");
				logger.info("参数heducationid【"+heducationidStr+"】应为正整数");
				value.setSuccess(false);
				out.write(JSON.toJSONString(value));
				return null;
    		}
    		...
    		/*调用获取健教详情    begin*/
    		Integer heducationid = Integer.valueOf(heducationidStr);
			HealthEducation info = healthFacedeService.selectHealthById(heducationid);
			String newContent = "没有健教内容";
    		if(info != null){
    			...
    			mav.setViewName("healthEducation"); 
    			return mav;
    		}else{
    			mav.addObject("data", newContent);
    			mav.setViewName("healthEducation"); 
    			return mav;
    		}
    		/*调用获取健教详情    end*/
