...
<head>
	...
	<script type="text/javascript" src="<%=path %>/js/singleOppgRep.js"></script>
	...
	<form action="<%=basePath%>/singleReport/saveSingleAuditContent" method="post" onsubmit="javascript: editor.sync();return checkAudit();" enctype="application/x-www-form-urlencoded">
		 ...
		 <table class="table-border">
			...
			<tr>
				...
				<td><fm:formatDate value="${omrr.grenerTime }" type="date" dateStyle="default"/></td>
				<td>报告审核时间：</td>
				<td><fm:formatDate value="${oasr.auditDatetime }" type="date" dateStyle="default"/></td>
			</tr>
		</table>
		<br />
		<div id="chart">
		...
		<table class="table-border">
			...
			<tr>
				...
				<td>
					<a href="" onclick="scanXdDetail('${omem.memname}','${item.memberid }','${omem.gender }','${omem.birthdate }','${omem.headaddress}','${item.eventid}');">查看</a>
				</td>
			</tr>
			</c:forEach> 
		</table>
			...
			<div class="result-list-box">	
				...
				<div class="result-list">
					<c:if test="${fn:length(smr1Others) != 0}">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;
						<%-- <img src="${pageContext.request.contextPath }/image/imageAction!getImage?imageId=${item.signaddress}"/> --%>
						<br/>
