	...
	<script type="text/javascript">
		...
		//置灰效果
			...
			$("#lines").change(function(){
				var lines=$("#lines").val();
				window.location = "${pageContext.request.contextPath}/singleReport/showSingAduitList?funId=1&pageSize="+lines;
			});
			...
			$("#toPage-btn").click(function(){
				...
				var topage= $("#toPage");
				topage.val(pageNow);
				window.location = "${pageContext.request.contextPath}/singleReport/showSingAduitList?funId=1&pageNo="+num+"&pageSize="+lines;
			});
			//禁用分页回车键
			...
			$("#page-first").click(function(){
				...
				window.location = "${pageContext.request.contextPath}/singleReport/showSingAduitList?funId=1&pageNo=1&pageSize="+lines;
			});
			$("#pre").click(function(){
			    var lines=$("#lines").val();
				var pageNum= "${page.pageNo}";
				...
				window.location = "${pageContext.request.contextPath}/singleReport/showSingAduitList?funId=1&pageNo=${page.pageNo-1}&pageSize="+lines;
			}); 
			$("#nex").click(function(){
			    var lines=$("#lines").val();
				var pageNum= "${page.pageNo}";
			...
			$("#page-last").click(function(){
				...
				window.location = "${pageContext.request.contextPath}/singleReport/showSingAduitList?funId=1&pageNo=${page.totalPages}&pageSize="+lines;
			}); 
		});
	</script>
	<style>
...
<body>
	<div class="content">
		<div class="content-title">单项测量审核记录列表</div>
					...
					<tbody>
						<c:forEach var="item" items="${page.result}" varStatus="status">
							<tr>
				              ...
				              <!-- <td><s:property value="getMultTempName(tempCode)"/></td> -->
				              ...
				              <td>
				               ...
				               <td>
				               		<a href="${pageContext.request.contextPath}/singleReport/toSingleReportAuditPage?reportNo=${item.reportNo}&serialNumber=${item.serialNumber}">${item.auditState =='N'?'立即处理':'继续处理' }</a>
				               </td>
				         	</tr>
				         </c:forEach>  
			         </tbody>
