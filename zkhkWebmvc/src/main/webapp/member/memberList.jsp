...
<div class="content">
	<c:if test="${flag eq 'other'}">
		<div class="content-title">其他会员</div>
	</c:if>
	<c:if test="${flag ne 'other'}">
		<div class="content-title">所有会员</div>
	</c:if>
			...
			<ul class="search-wrapper">
				<c:if test="${flag eq 'my'}">
					<li>
						...
						<input class="info info-search" type="text" name ="memberGroupName" value="${param.memberGroupName}" id="memberGroupName" readonly="readonly" />
	                 	<i class="icon icon-group-search" id="showGroupIcon" onclick="showGroups();"></i>	              
					</li>
				</c:if>
				<li>
				...
				<li>
					<span class="label">疾病状况：</span>
	                    <input class="info info-illness" type="text" name="diseaseName" value="${condition.diseaseName }" maxlength="30">	
				</li>
				<li>
				...
				<%--<li>
					<span class="label label-5x">地址:</span>
	                <input class="info info-2x" type="text" name="omem.address" value="<s:property value='omem.address'/>"/>
				</li>--%>
				<li>
							...
							<tr id="tr_${item.memberId}">
								...
								<c:if test="${flag eq 'my'}">
									<td id="table_content_title_td">
										...
										<a href="javascript: void(0);" onclick="updateMemberGroup(${item.memberId},'${item.memName}','${item.memGrpIDs}')">修改所属分组</a>&nbsp;
									</td>
								</c:if>
								<c:if test="${flag eq 'other'}">
									<td id="table_content_title_td">
