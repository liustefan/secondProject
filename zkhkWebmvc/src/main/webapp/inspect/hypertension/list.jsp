...
<head>
	...
	function delHypertension() {
		...
		if (checkList.length <= 0) {
			layer.alert('请选择高血压随访信息', {skin: 'skin1', shade: 0})
			return;
		}
...
<%-- 							 <c:choose> --%>
<%--   								<c:when test="${isMy || empty pojo.memberID}"> --%>
<!-- 									<button type="button" id="" class="btn-inquiry ico-del font-change" onclick="delHypertension()">删除</button> -->
<%-- 								</c:when> --%>
<%-- 								<c:otherwise> --%>
...
<%-- 				    		</c:if> --%>
			    		...
			    		<tbody>
				    		<c:forEach items="${page.result}" var="item" varStatus="vs">
					    		<tr>
<%-- 					    			<c:if test="${!pojo.pending}"> --%>
<%-- 					    			<td><input type="checkbox" name="ids" value="${item.hypertensionID}" ${item.refCompany==0 ? '':'disabled'}></td> --%>
<%-- 					    			</c:if> --%>
					    			<c:if test="${empty pojo.memberID}">
					    			<td><a href="<%=path%>/member/${item.member.memberid}/memberPage?page=followUp1" target="_new">${item.member.memname}</a></td>
...
<%-- 					    			<c:choose> --%>
<%-- 				    					<c:when test="${pojo.pending}"> --%>
<%-- 				    						<a href="toEdit?id=${item.hypertensionID}&singleMembers=${empty pojo.memberID ? false : true}">进行随访</a> --%>
<%-- 				    					</c:when> --%>
<%-- 				    					<c:otherwise> --%>
				    						<a href="view?id=${item.hypertensionID}&memberID=${pojo.memberID}">查看</a>&nbsp;
<%-- 					    					<c:if test="${isMy || empty pojo.memberID}"> --%>
<%-- 						    					<c:if test="${item.refCompany==0}"> --%>
<%-- 						    						<a class="mar-left" href="toEdit?id=${item.hypertensionID}&singleMembers=${empty pojo.memberID ? false : true}">修改</a>&nbsp; --%>
<%-- 						    					</c:if> --%>
<%-- 							    				<c:if test="${item.refCompany!=0}"> --%>
