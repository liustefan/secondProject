...
<head>
	...
	$(function() {
		...
		checkIsDisplayCard();
		$("input[name='isDisplayCard']").click(function(){
			checkIsDisplayCard();
		});
	});
	</script>
								       ...
								       <li>
										<input name="memMustSetItemStr" type="checkbox" value="${setting.num }" id="checkboxId_${setting.num}"  
 										<c:forEach items="${org.orgConfig.memMustSetItemArr }" var="memMustSetItem"> 
 											<c:if test="${memMustSetItem eq setting.num }">checked="checked"</c:if>
 										</c:forEach> />${setting.text }
									  </li>
							 </c:forEach>
						</ul>
					</div>
					...
					<div class="fl" id="member-type">
       					<div class="fl">
       					    <label class="radio-inlines fixed-width">
	       						<input type="radio" name="isDisplayCard" value="1" 
								<c:if test="${org.orgConfig.isDisplayCard eq '1'}">checked="checked"</c:if>><span>是</span>
							</label> 
							<label class="radio-inlines fixed-width">
	       						<input type="radio" name="isDisplayCard" value="0" 
								<c:if test="${org.orgConfig.isDisplayCard eq '0' || empty org.orgConfig.isDisplayCard }">checked="checked"</c:if>><span>否</span>
							</label>
                		</div>
					</div>
				</div>
