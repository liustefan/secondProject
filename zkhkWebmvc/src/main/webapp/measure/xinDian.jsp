		...
		$(function(){
			$("#single").click(function(){
				if($("#innerIframe1").attr('src') == ''){
					$("#innerIframe1").attr('src','<%=path %>/electrocardio/showSingleElectrocardio?memberId=${param.memberId }&flag=${param.flag}')
				}
			})
		});
...
<body>
<div class="content" <c:if test="${not empty param.eventId}">id="content100"</c:if>>
	<c:if test="${empty param.eventId}">
		<div class="content-title">动态心电测量</div>
	</c:if>
	<nav>
	...
	<div class="tab-content">
		<div id="tab_conbox" class="clearfix">
			...
			<section class="tab-conbox-item">
				<iframe id="innerIframe2" name="innerIframe2" width="100%" height="" src="<%=path %>/electrocardio/showAllElectrocardio?memberId=${param.memberId }&flag=1"></iframe>
			</section>
		</div>
	</div>    
</div>
