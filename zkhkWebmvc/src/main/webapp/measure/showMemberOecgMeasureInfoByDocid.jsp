	...
	var index = layer.getFrameIndex(window.name); //获取窗口索引
		$(function(){
			...
			//置灰效果
	          ...
	          $("#pageSize").change(function(){
	        	  var pageSize=$("#pageSize").val();
	 			  window.location = "../electrocardio/queryElectrocardio?&wheAbnTag=${wheAbnTag}&docid=${userInfo.id }&pageSize="+pageSize+"&criteria="+encodeURI(encodeURI("${criteria}"));
	          });
			 ...
			 $("#page-first").click(function(){
				 var pageSize=$("#pageSize").val();
				  ...
				  window.location = "../electrocardio/queryElectrocardio?wheAbnTag=${wheAbnTag}&docid=${userInfo.id }&pageNo=1&criteria="+encodeURI(encodeURI("${criteria}"))+"&pageSize="+pageSize;
	          });
			...
			$("#page-last").click(function(){
				...
				window.location = "../electrocardio/queryElectrocardio?wheAbnTag=${wheAbnTag}&docid=${userInfo.id }&pageNo=${page.totalPages}&criteria="+encodeURI(encodeURI("${criteria}"))+"&pageSize="+pageSize; 	
			});
			...
			$("#pre").click(function(){
				var pageSize=$("#pageSize").val();
			  	...
			  	window.location = "../electrocardio/queryElectrocardio?wheAbnTag=${wheAbnTag}&docid=${userInfo.id }&pageNo=${page.pageNo-1}&criteria="+encodeURI(encodeURI("${criteria}"))+"&pageSize="+pageSize; 
			});
			...
			$("#toPage-btn").click(function(){
				...
				if(num>totalPage){
					...
					var topage= $("#toPage");
					topage.val(pageNow);
				  	window.location = "../electrocardio/queryElectrocardio?wheAbnTag=${wheAbnTag}&docid=${userInfo.id }&pageNo=" + num + "&criteria="+encodeURI(encodeURI("${criteria}"))+"&pageSize="+pageSize;			
				}
			});
			
			$("#select1").click(function(){
				var criteria=$("#find").val();
				window.location = "../electrocardio/queryElectrocardio?wheAbnTag=${wheAbnTag}&docid=${userInfo.id}&criteria="+encodeURI(encodeURI(criteria));
			});
		...
		function deleteInfo(docid, wheAbnTag){
			...
			if(checkboxs <= 0){
				...
				if (confirm("确定要删除这些测量信息吗？")) {
					...
					if(wheAbnTag == 1){
						window.location = "../electrocardio/deleteBatchElectrocardioInfo?parameter="+s+"&wheAbnTag=1&docid="+docid;
					}else{
						window.location = "../electrocardio/deleteBatchElectrocardioInfo?parameter="+s+"&wheAbnTag=0&docid="+docid;
					}
				}else{
					return false;
				}
		...
		function scanXdDetail(memName,memberId,gender,birthDate,headAddress,eventId){
			var url = "../measure/showMeasXindian.jsp?memName="+encodeURI(encodeURI(memName))+"&memberId="+memberId+"&gender="+gender+"&birthDate="+birthDate
...
<div class="content">
	...
	<ul class="content-tabs">
		<li id="footer_content_ul_1">
			<a <c:if test="${wheAbnTag ==1 }">class="current"</c:if> 
			href="../electrocardio/queryElectrocardio?wheAbnTag=1&docid=${userInfo.id }">异常测量数据</a>
		</li>
		<li>
			<a <c:if test="${wheAbnTag !=1 }">class="current"</c:if>
			href="../electrocardio/queryElectrocardio?docid=${userInfo.id }">所有测量数据</a>
		</li>
	</ul>	
				...
				<table class="table-content">
					...
					<c:forEach items="${page.result }" var="item">
							<tr>
								...
								<td>${item.averageheart }</td>
								<td><fmt:formatDate value="${item.meastime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
								<td>
									<a href="javascript:void(0);" onclick="scanXdDetail('${item.memName}','${item.memberid }','${item.gender }','${item.birthDate }','${item.headAddress}','${item.eventid}');">查看</a>
									<c:choose>
										<c:when test="${wheAbnTag == 1 }">
											<a class="mar-left" onclick="return confirm('确认删除吗？');" href="../electrocardio/deleteElectrocardioInfo?docentry=${item.docentry }&wheAbnTag=1&docid=${userInfo.id}">删除</a>
										</c:when>
										<c:otherwise>
											<a class="mar-left" onclick="return confirm('确认删除吗？');" href="../electrocardio/deleteElectrocardioInfo?docentry=${item.docentry }&wheAbnTag=0&docid=${userInfo.id}">删除</a>
										</c:otherwise>
									</c:choose>
								</td>
							</tr>
