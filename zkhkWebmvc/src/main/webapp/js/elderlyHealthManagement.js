...
//老年人接受健康管理状况
function drawHealthManage(obj, length) {
    $('#chart-healthManage').highcharts({
        chart: {
            type: 'bar',
            borderWidth: 1,
            borderColor: '#ccc',
...
function onloadTable(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        obj = list[i];
        result += '<tr><td>' + obj.placeName + '</td><td>' + obj.total + '</td><td>' + obj.noHealthManage + '</td></tr>';
    }
...
function initPage(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        list1.noHealthManage.push(obj.noHealthManage);
    }
    //填充table
    onloadTable(data);
    //各地区老人体检管理状况表
    drawHealthManage(list1, dataCount);
}

function clean() {
    $("#datalist tbody").empty();
    $("#chart-healthManage").empty();
}
...
function search() {
    ...
    if (orgId) {
        $(".loadBox").show();
        $.get("../statistic/exProSelectElderlyHealthManagement", { orgId: orgId, year: year }, function(data) {
            $(".loadBox").hide();
            if (data.status) {
                // 先清空之前的数据以及图表
                clean();
