...
function SelectDate(obj, strFormat, x, y) {
	...
	var date = new Date();
	var by = date.getFullYear() - 100; // 最小值 → 50 年前
	//var ey = date.getFullYear() + 50; // 最大值 → 50 年后
	
	//由sz添加 2015-05-05 
	var ey = date.getFullYear(); // 最大值为当前年份
	//var em = date.getFullMonth();//获取当前月份(0-11,0代表1月)    
    //var ed = date.getDate();//获取当前日(1-31)
...
Calendar.prototype.ReturnDate = function(dt) {
	...
	ev = ev.substring(((ev.indexOf("ValidatorOnChange();") > 0) ? ev
			.indexOf("ValidatorOnChange();")
			+ 20 : ev.indexOf("{") + 1), ev.lastIndexOf("}"));// 去除验证函数
																// ValidatorOnChange();
...
function Calendar(beginYear, endYear, lang, dateFormatStyle) {
	...
	/*
	 * //2008-01-29 放到了 show ，因为要做 pickMode 判断 this.draw(); this.bindYear();
	 * this.bindMonth();
	 */
	// this.changeSelect();
...
Calendar.prototype.draw = function() {
	...
	mvAry[mvAry.length] = '      <select name="calendarHour" id="calendarHour" style="font-size:12px;"></select>'
			+ Calendar.language["hour"][this.lang];
	...
	// 精确到小时时隐藏
	// “分”
	mvAry[mvAry.length] = '"><select name="calendarMinute" id="calendarMinute" style="font-size:12px;"></select>'
			+ Calendar.language["minute"][this.lang] + '</span>';
	mvAry[mvAry.length] = '<span style="'
	...
	// 精确到小时、分时隐藏
	// “秒”
	mvAry[mvAry.length] = '"><select name="calendarSecond" id="calendarSecond" style="font-size:12px;"></select>'
			+ Calendar.language["second"][this.lang] + '</span>';
	mvAry[mvAry.length] = '      </td></tr>';

	mvAry[mvAry.length] = '    </table>';
	// mvAry[mvAry.length] = ' </from>';
	mvAry[mvAry.length] = '      <div align="center" style="padding:4px 4px 4px 4px;background-color:'
	...
	mvAry[mvAry.length] = (calendar.DateMode == pickMode["day"])
		...
		if (dateArray[i] != "&nbsp;") {
			...
			tds[i].onclick = function() {
															...
															// 当选择日期时，点击格子即返回值
				{
					...
					{
						calendar.selectedDayTD.style.backgroundColor = (calendar.selectedDayTD.isToday)
...
Calendar.prototype.show = function(dateObj, popControl) {
	...
	this.date = (dateObj.value.length > 0) ? new Date(dateObj.value
			...
			.toDate(this.dateFormatStyle);// 2008-01-29
	// 寒羽枫添加
	// →
	// 若为空则根据dateFormatStyle初始化日期

	if (this.panel.innerHTML == ""
			|| cal.dateFormatStyleOld != cal.dateFormatStyle)// 2008-01-29
	// 把构造表格放在此处，2009-03-03
	// 若请示的样式改变，则重新初始化
	{
		this.draw();
		this.bindYear();
	...
	this.panel.style.left = (xy.x + leftX) + "px"; // 由寒羽枫 2007-02-11 修改 →
													// 加入自定义偏移量
	...
	if (!this.dateControl.isTransEvent) {
		...
		/*
		 * 已写在返回值的时候 ReturnDate 函数中，去除验证事件的函数 this.dateControl.changeEvent =
		 * this.dateControl.onchange;//将 onchange 转成其它函数，以免触发验证事件
		 * this.dateControl.onchange = function() {if(typeof(this.changeEvent)
		 * =='function'){this.changeEvent();}}
		 */
