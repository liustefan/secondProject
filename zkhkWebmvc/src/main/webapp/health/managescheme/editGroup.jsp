...
<head>
	...
	<style type="text/css">
		...
		.tab>ul>li.tab2 {
			left: 220px;
			background-image: url("<%=path %>/img/tab-bg_.png");
		}
		
		.tab>ul>li.tab2.active {
			background-image: url("<%=path %>/img/tab-bg.png");
		}
...
// 				$("#chooseMode").hide();
	        	...
	        	if(scope.active.willNext){
	        		scope.active = {activeTab: scope.active.willNext, willNext: false};
	        	}else{
	        		alert("保存成功");
	        	}
	        	if(tasks)
		...
		function chooseMemberCallback(memberId, serialize){
			closeAllLayer();
			$.ajax({
				   type: "POST",
				   url: memberId ? "addGroupMember" : "addGroupMembers",
				   asyn: false,
				   data: memberId ? {memberID: memberId, MSDesignID: angular.element("input[name='MSDesignID']").val()} : serialize,
				   success: function(response){
					   if(response.status){
						   var src = 'listGroup_members?MSDesignID=' + $('input[name="MSDesignID"]').val() + '&massStatus=${pojo.massStatus}&massEffectProcess=${pojo.massEffectProcess}&view=${param.view}';
						   $("#listPerson").attr("src", src);
					   }else{
						   alert("添加会员失败，请稍候再试！");
					   }
				   },
		...
		app.controller('ManageModeCtrl', function ($scope, $http, $filter) {
			$scope.active = {activeTab: '${pojo.massStatus eq 2 and !param.view}' == 'true' ? 4 : 1,  willNext: '${pojo.massStatus eq 2 and !param.view}' == 'true' ? true : false};
			$scope.dict = {};
			$scope.task = {};
		  	$scope.event = {
		  			init: function(){
					...
					save: function(next){
						if(next === 2 || next === 4){//点击下一步：保存基本资料成功之后进入下一步
	//							this.checkTitleUnique(function(){
								if(vForm.form()){
									$scope.active.willNext = next;
									$('form')[0].submit();
								}
	//							});
						}else{
	...
	//								});
					...
					savePersonTask: function(){
						...
						var $this = this;
						this.callRemote("saveGroupTask", $scope.task.add, function(reponse){
							$("#save").prop('disabled',false);
							if(reponse.status){
				    ...
				    deleteTask: function(id, index){
				    	this.callRemote("deleteTask", {id: id, type: 2}, function(reponse){
				    		if(reponse.data){
				    ...
				    saveEditPlanTime: function(){
				    	...
				    	var data = {MSDTaskID: $scope.task.add.mSDTaskID, type: 2};
				    ...
				    groupEffect: function(e){
				    	if(e.target.value == '方案生效且任务生成'){
				    		if(confirm("管理方案生效后不能再修改方案任务，您确定要生效吗？")){
				    			this.callRemote("groupEffect", {MSDesignID: this.getMSDesignID()}, function(reponse){
						    		if(reponse.status){
						    			window.location.href = 'listGroup';
									}else{
										alert(reponse.content);
									}
						    	});
		...
		function openQuestion(){
			...
			layer.open({
		   	    ...
		   	    shade: 0,
		   	 	area: ['860px', '520px'],
		   	    content: 'chooseQuestionnaire.html', //iframe的url
		   	}); 
		}
	 	...
	 	function chooseMember(MSDesignID) {
	 		if(!MSDesignID)
	 			MSDesignID = $('input[name="MSDesignID"]').val();
			layer.closeAll();
			layer.open({
		   	    type: 2,
		   		skin : 'skin1',
		   	    ...
		   	    shade: 0,
		   	 	area: ['900px', '485px'],
		   	    content: 'listMember?iMSDesignID='+MSDesignID+"&iDocId="+ $('input[name="execDrID"]').val()//iframe的url
		   	}); 
		}
	 	$(function(){
	 		$("#price").hide();
 		...
 		$(function(){
			if('${pojo.massStatus eq 2 or param.view}' == 'true'){
...
<body ng-controller="ManageModeCtrl">
	<div class="content">
		<div class="content-title">群体管理方案 --- <c:if test="${empty pojo.MSDesignID}">新增</c:if><c:if test="${not empty pojo.MSDesignID and !param.view}">修改</c:if><c:if test="${not empty pojo.MSDesignID and param.view}">查看</c:if></div>
		<div class="tab">
			<ul>
				<li class="tab1" ng-class="{true: 'active'}[active.activeTab == 1]" data-page="tab1-page" ng-click="event.changeTab(1)">1 方案基本资料</li>
				<li class="tab2" ng-class="{true: 'active'}[active.activeTab == 2 || active.activeTab == 3]" data-page="tab2-page" ng-click="event.changeTab(2, null, '${pojo.massStatus eq 2 or param.view}')">2 方案执行任务</li>
				<li class="tab3" ng-class="{true: 'active'}[active.activeTab == 4]" data-page="tab3-page" ng-click="event.changeTab(4, null, '${pojo.massStatus eq 2 or param.view}')">3 选择会员</li>
			</ul>
		...
		<div class="main-content">
			...
			<div id="tab1-page" ng-show="active.activeTab == 1">
				<form id="form_1" name="form_1" action="save" novalidate ng-disabled="operate=='view'" target="upload" method="post" enctype="multipart/form-data">	
					...
					<input type="hidden" id="massEffectProcess" value="${pojo.massEffectProcess}">
					<table border="0" cellspacing="0" class="table-normal table-fixed">
			    		<tbody>
				    		<tr>
				    			<td colspan="1"><span class="red">*</span>方案标题：</td>
				    			<td colspan="6">
				    				<input type="text" class="width-lg" name="schemeTitle" tName="schemeTitle" value="${pojo.schemeTitle}" maxlength="20">
			    				</td>
				    		</tr>
				    		<tr>
				    		...
				    		<tr>
				    			...
				    			<td colspan="6">
				    				<div class="position-r">
						                ...
						                <input type="text" name="execDrName" readonly id="execDrName" value="${pojo.execDrName}">
						                <i class="member-search2" onclick="chooseDoc('${pojo.massStatus eq 2 or param.view}');"></i>
				                 	</div>
			...
			<div id="tab2-page" ng-cloak ng-show="active.activeTab == 2">
				<form id="form2" name="form_2" action="" novalidate>
					<div class="clearfix content-head">
						...
						<c:if test="${pojo.massStatus eq 1 and !param.view}">
			   		    	...
			   		    	<tbody>
			   		    		<tr ng-repeat="task in task.list">
			   		    			...
			   		    			<td>
			   		    				...
			   		    				<a href="javascript:void(0);" ng-click="event.deleteTask(task.mSDTaskID, $index)" >删除</a>
			   		    			</td>
			   		    			</c:if>
			   		    		</tr>
			   		    	</tbody>
...
<!-- 			                </tr> -->
			...
			<div id="tab3-page" style="height: 450px" ng-show="active.activeTab == 4">
				<iframe id="listPerson" width=100% height=100% frameborder="0" src="listGroup_members?MSDesignID=${pojo.MSDesignID}&massStatus=${pojo.massStatus}&massEffectProcess=${pojo.massEffectProcess}&view=${param.view}"></iframe>
			</div>
		</div>
		<c:if test="${!param.view and pojo.massStatus ne 2}">
			<div class="page-box" ng-class="{true: 'align-center'}[active.activeTab == 3]">
				<input type="button" ng-show="active.activeTab == 3" class="btn-inquiry" id="save" ng-click="event.save()" value="保存"/>
				<input type="button" ng-show="active.activeTab == 3" class="btn-cancel" value="取消" ng-click="event.cancleEditTask()"/>
				<c:if test="${pojo.massEffectProcess eq 2}">
					<input type="button" ng-show="active.activeTab == 4" class="btn-cancel fr" id="" value="生成中"/>
				</c:if>
				<c:if test="${pojo.massEffectProcess ne 2}">
					<input type="button" ng-show="active.activeTab == 4" class="btn-inquiry fr" id="" ng-click="event.groupEffect($event)" value="${pojo.massStatus eq 1 ? '方案生效且任务生成' : '任务生成'}"/>
				</c:if>
				<input type="button" ng-show="active.activeTab == 1" class="btn-cancel" value="返回" onclick="window.location.href='${empty param.source ? 'listGroup' : param.source}'"/>
				<input type="button" ng-show="active.activeTab == 2 || active.activeTab == 4" class="btn-inquiry fl" value="上一步" ng-click="event.changeTab(active.activeTab - (active.activeTab == 4 ? 2 : 1))"/>
				<input type="button" ng-show="active.activeTab == 1 || active.activeTab == 2" class="btn-inquiry fr" value="下一步" ng-click="event.changeTab(active.activeTab + (active.activeTab == 2 ? 2 : 1))"/>
			</div>
		</c:if>
		<c:if test="${param.view}">
			<div class="page-box align-center">
				<input type="button" class="btn-cancel" value="返回" onclick="window.location.href='${empty param.source ? 'listGroup' : param.source}'"/>
			</div>
		</c:if>
		<c:if test="${not empty pojo.MSDesignID and !param.view and pojo.massStatus eq 2}">
			<div class="page-box align-center" ng-show="active.activeTab == 1 || active.activeTab == 2">
				<input type="button" class="btn-cancel" value="返回" onclick="window.location.href='${empty param.source ? 'listGroup' : param.source}'"/>
			</div>
			<div class="page-box">
				<input type="button" ng-show="active.activeTab == 4" class="btn-cancel fl" value="返回" onclick="window.location.href='${empty param.source ? 'listGroup' : param.source}'"/> 
				<c:if test="${pojo.massEffectProcess eq 2}">
					<input type="button" ng-show="active.activeTab == 4" class="btn-cancel fr" id="" value="生成中"/>
				</c:if>
				<c:if test="${pojo.massEffectProcess ne 2}">
	...
	<form action="" method="post">
		<div class="layer-box">
			<span class="red">*</span>任务计划时间： 方案触发时间后
			<input type="text" id="sure_time" class="width-number" name="planTimeValue" ng-model="task.add.planTimeValue">
			<select name="planTimeType" ng-model="task.add.planTimeType" ng-options="d.code as d.name for d in dict.planTimeType">
			</select>
		</div>
		<div class="page-box align-center">
