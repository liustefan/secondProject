/*
 * PhHealthexamdetailnonimmuneMapper.java
 * Copyright(C) 2016-2026  深圳中科汇康技术有限公司
 * All rights reserved.
 * -----------------------------------------------
 * 2016-06-27 Created
 */
package com.bithealth.inspectCore.physical.dao;

import com.bithealth.inspectCore.physical.model.PhHealthexamdetailnonimmune;
import com.bithealth.inspectCore.physical.model.PhHealthexamdetailnonimmuneExample;
import com.bithealth.sdk.core.generic.GenericBaseDao;

public interface PhHealthexamdetailnonimmuneMapper extends GenericBaseDao<PhHealthexamdetailnonimmune, PhHealthexamdetailnonimmuneExample, Long> {
}